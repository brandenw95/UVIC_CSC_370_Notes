...
- [Data Analysis with SQL (sql_time)](#data-analysis-with-sql--sql-time-)
  ...
  * [Results](#results-2)
  * [Chart](#chart-1)
- [Data Analysis with SQL Window Functions (sql_window)](#data-analysis-with-sql-window-functions--sql-window-)
  * [Product – Order line – Orders](#product---order-line---orders)
  * [orders per month per category](#orders-per-month-per-category)
  * [WINDOW FUNCTIONS](#window-functions)
    + [First: Mix detail and aggregate information over window](#first--mix-detail-and-aggregate-information-over-window)
    + [Second: Extract what you want with enclosing query](#second--extract-what-you-want-with-enclosing-query)
    + [Several levels of aggregations at once](#several-levels-of-aggregations-at-once)
    + […without window functions](#-without-window-functions)
- [Storage Mechanics (storage)](#storage-mechanics--storage-)
  * [Computer Quantities](#computer-quantities)
  * [Storage mechanics](#storage-mechanics)
  * [A Brief History of Disk Drives](#a-brief-history-of-disk-drives)
  * [HDDs today](#hdds-today)
  * [SSDs](#ssds)
  * [Similarities](#similarities)
  * [I/O model of computation](#i-o-model-of-computation)
  * [HDD USE CASE](#hdd-use-case)
    + [Disks](#disks)
    + [Tracks and sectors](#tracks-and-sectors)
    + [MEGATRON 747 Disk Parameters](#megatron-747-disk-parameters)
    + [Disk access time](#disk-access-time)
    + [Cause of rotational delay](#cause-of-rotational-delay)
    + [MEGATRON 747 Timing Example](#megatron-747-timing-example)
    ...
    + [AVG time to read a 16,384-byte block](#avg-time-to-read-a-16-384-byte-block-1)
    + [AVG time to read a 16,384-byte block](#avg-time-to-read-a-16-384-byte-block-2)
    + [Writing and Modifying Blocks](#writing-and-modifying-blocks)
  * [SORTING IN EXTERNAL STORAGE](#sorting-in-external-storage)
    + [Using Secondary Storage Effectively](#using-secondary-storage-effectively)
    + [Merge Sort](#merge-sort)
    + [TwoPhase, Multiway Merge Sort](#twophase--multiway-merge-sort)
    + [Phase 1](#phase-1)
    + [Phase 2](#phase-2)
    + [Phase 2 in words](#phase-2-in-words)
    + [Real Life Example](#real-life-example)
    + [Analysis – Phase 1](#analysis---phase-1)
    + [Analysis – Phase 2](#analysis---phase-2)
    + [How Big Should Blocks Be?](#how-big-should-blocks-be-)
    + [Another example: Block Size = 512K](#another-example--block-size---512k)
    + [Reasons to limit the block size](#reasons-to-limit-the-block-size)
    + [How many records can we sort?](#how-many-records-can-we-sort-)
    + [Sorting larger files](#sorting-larger-files)
  * [Toy example for 2PMMS](#toy-example-for-2pmms)
    + [Toy Example](#toy-example)
- [BTree Indexes (btrees)](#btree-indexes--btrees-)
  * [Why Indexes?](#why-indexes-)
  * [BTrees, Un-clustered index: A leaf and interior node](#btrees--un-clustered-index--a-leaf-and-interior-node)
  * [Clustered index: Leaf with records and interior node](#clustered-index--leaf-with-records-and-interior-node)
  * [Types of B-Trees](#types-of-b-trees)
  * [Operations in B-Tree](#operations-in-b-tree)
    + [Lookup](#lookup)
    + [Insertion](#insertion-1)
    + [Beginning of the insertion of key 40](#beginning-of-the-insertion-of-key-40)
    + [Continuing of the Insertion of key 40](#continuing-of-the-insertion-of-key-40)
    + [Completing of the Insertion of key 40](#completing-of-the-insertion-of-key-40)
    + [Completing the Insertion of key 40](#completing-the-insertion-of-key-40)
    + [Deletion](#deletion-1)
    + [Deletion (Raising a key to parent)](#deletion--raising-a-key-to-parent-)
    + [Deletion](#deletion-2)
    + [Deletion](#deletion-3)
    + [Deletion](#deletion-4)
  * [Structure of B-trees with real blocks](#structure-of-b-trees-with-real-blocks)
  * [Example](#example-16)
  * [Building a B-Tree from Existing Data](#building-a-b-tree-from-existing-data)
  * [Pointer Intersection](#pointer-intersection)
- [Query Evaluation (queryeval)](#query-evaluation--queryeval-)
  * [An SQL query and its RA equiv.](#an-sql-query-and-its-ra-equiv)
  * [Query Optimization](#query-optimization)
  * [Running Example – Airline](#running-example---airline)
  * [Algorithms for selection](#algorithms-for-selection)
  * [Algorithms for projection](#algorithms-for-projection)
  * [Algorithms for joins](#algorithms-for-joins)
  * [Algorithms for joins (merge-join)](#algorithms-for-joins--merge-join-)
  * [Algorithms for joins (sort-merge)](#algorithms-for-joins--sort-merge-)
  * [Optimization](#optimization)
  * [Query evaluations plans](#query-evaluations-plans)
  * [Alternative query evaluation plans](#alternative-query-evaluation-plans)
    + [Plan: Pushing selections I](#plan--pushing-selections-i)
    + [Plan: Pushing selections II](#plan--pushing-selections-ii)
    + [Plan: Pushing selections III](#plan--pushing-selections-iii)
    + [Plan: Pushing selections IV](#plan--pushing-selections-iv)
  * [Pushing projections](#pushing-projections)
    + [Plan using Indexes I](#plan-using-indexes-i)
    + [Plan using Indexes II](#plan-using-indexes-ii)
    + [Plan using Indexes III](#plan-using-indexes-iii)
    + [Plan using Indexes IV](#plan-using-indexes-iv)
    + [Plan using Indexes V](#plan-using-indexes-v)
- [Transactions (Concurrency) - Controlling Concurrent Behavior](#transactions--concurrency----controlling-concurrent-behavior)
  * [But what’s a transaction?](#but-what-s-a-transaction-)
  * [Another example of a transaction](#another-example-of-a-transaction)
  * [SQL statements: COMMIT and ROLLBACK](#sql-statements--commit-and-rollback)
  * [Busy, busy, busy...](#busy--busy--busy)
  * [Concurrent Transactions](#concurrent-transactions)
  * [ACID Transactions](#acid-transactions)
  * [Transactions and Schedules](#transactions-and-schedules)
  * [Anomalies: Reading Uncommitted Data](#anomalies--reading-uncommitted-data)
  * [Anomalies: Unrepeatable Reads](#anomalies--unrepeatable-reads)
  * [Anomalies: Overwriting uncommitted data](#anomalies--overwriting-uncommitted-data)
  * [Summarizing the Terminology](#summarizing-the-terminology)
  * [Swaps and Conflicts](#swaps-and-conflicts)
    + [Example](#example-17)
  * [Serializability/precedence Graphs](#serializability-precedence-graphs)
  * [Precedence graphs](#precedence-graphs)
  * [Why the Precedence-Graph Test Works?](#why-the-precedence-graph-test-works-)
  * [Schedulers](#schedulers)
  * [Lock Actions](#lock-actions)
  * [Validity of Locks](#validity-of-locks)
  * [Legal Schedule Doesn’t Mean Serializable](#legal-schedule-doesn-t-mean-serializable)
  * [Two Phase Locking](#two-phase-locking)
  * [Why 2PL Works?](#why-2pl-works-)
  * [What should we lock?](#what-should-we-lock-)
  * [Transaction support in SQL](#transaction-support-in-sql)
  * [In Summary](#in-summary)
  * [Optional: Conflict-serializability vs serializability](#optional--conflict-serializability-vs-serializability)
- [Types of Locks (Concurrency_lock_types)](#types-of-locks--concurrency-lock-types-)
  ...
  * [Solution: Update Locks](#solution--update-locks)
  * [Example](#example-18)
  * [(No) Deadlock Example](#-no--deadlock-example)
  * [Benefits of Upgrade Locks](#benefits-of-upgrade-locks)
- [Recovery from crashes (recovery)](#recovery-from-crashes--recovery-)
  * [ACID](#acid)
  * [Primitive DB Operations of Transactions](#primitive-db-operations-of-transactions)
  * [Example (Cont’d)](#example--cont-d-)
  * [Undo Logging](#undo-logging)
  * [Undo Logging (Cont’d)](#undo-logging--cont-d-)
  * [Example:](#example-)
  * [Recovery With Undo Logging](#recovery-with-undo-logging)
  * [Example](#example-19)
  * [Checkpointing](#checkpointing)
  * [Example of an Undo log with CKPT](#example-of-an-undo-log-with-ckpt)
  * [Nonquiescent Checkpoint (NQ CKPT)](#nonquiescent-checkpoint--nq-ckpt-)
  * [Recovery with NQ CKPT](#recovery-with-nq-ckpt)
  * [Example of NQ Undo Log](#example-of-nq-undo-log)
  * [Undo Drawback](#undo-drawback)
  * [Redo Logging](#redo-logging)
  * [Redo Logging (Cont’d)](#redo-logging--cont-d-)
  * [Example REDO:](#example-redo-)
  * [Compare to UNDO](#compare-to-undo)
  * [Recovery With Redo Logging](#recovery-with-redo-logging)
  * [Checkpointing for Redo Logging](#checkpointing-for-redo-logging)
  * [Checkpointing for Redo (Cont’d)](#checkpointing-for-redo--cont-d-)
  * [Checkpointing for Redo (Cont’d)](#checkpointing-for-redo--cont-d--1)
  * [Recovery with Ckpt. Redo](#recovery-with-ckpt-redo)
  * [Undo/Redo Logging](#undo-redo-logging)
  * [Undo/Redo Logging Scheme](#undo-redo-logging-scheme)
  * [Undo/Redo vs Undo and Redo](#undo-redo-vs-undo-and-redo)
  * [Simplified: Undo/Redo vs Undo and Redo](#simplified--undo-redo-vs-undo-and-redo)
  * [Example UNDO/REDO:](#example-undo-redo-)
  * [Compare to UNDO](#compare-to-undo-1)
  * [Compare to REDO:](#compare-to-redo-)
  * [Undo/Redo Recovery](#undo-redo-recovery)
  * [Undo/Redo Checkpointing](#undo-redo-checkpointing)
  * [Undo/Redo Recovery](#undo-redo-recovery-1)
  * [Example](#example-20)
- [Functional Dependencies and Boyce-Codd Normal Form (bcnf)](#functional-dependencies-and-boyce-codd-normal-form--bcnf-)
  * [Babies Schema](#babies-schema)
  * [Babies Table](#babies-table)
  * [Anomalies](#anomalies)
  * [A Fix](#a-fix)
  * [Alternative Fix](#alternative-fix)
  * [Functional Dependencies](#functional-dependencies)
  * [Another Example](#another-example-4)
  * [Example Data](#example-data)
  * [FD’s With Multiple Attributes](#fd-s-with-multiple-attributes)
  * [Keys of Relations](#keys-of-relations)
  * [Boyce-Codd Normal Form](#boyce-codd-normal-form)
  * [BCNF Violation - Example](#bcnf-violation---example)
  * [Decomposition into BCNF](#decomposition-into-bcnf)
  * [Babies Decomposition](#babies-decomposition)
  * [Babies Decomposition](#babies-decomposition-1)
  * [Deducing FDs](#deducing-fds)
  * [Closure of a Set of Attributes](#closure-of-a-set-of-attributes)
  * [Computing the Closure - Algorithm](#computing-the-closure---algorithm)
  * [Computing the Closure - Algorithm](#computing-the-closure---algorithm-1)
  * [Computing the Closure - Example](#computing-the-closure---example)
  * [Why Computing the Closure?](#why-computing-the-closure-)
  * [Example](#example-21)
  * [baby functionally determines doctor](#baby-functionally-determines-doctor)
  * [Closures and Keys](#closures-and-keys)
...
- [Lecture 1](assets/1.intro.pdf.pdf) - intro
- [Lecture 2](assets/2.er.pdf.pdf) - er
- [Lecture 3](assets/3.sql1.pdf.pdf) - sql1
- [Lecture 4](assets/4.er-rel.pdf.pdf) - er-rel
- [Lecture 5](assets/5.pharmacies_example.pdf.pdf) - pharmacies_example
- [Lecture 6](assets/6.rel-algebra.pdf.pdf) - rel-algebra
- [Lecture 7](assets/7.rel-algebra2.pdf.pdf) - rel-algebra2
- [Lecture 8](assets/8.sql2.pdf.pdf) - sql2
- [Lecture 9](assets/9.sql4.pdf.pdf) - sql4
- [Lecture 10](assets/10.sql3.pdf.pdf) - sql3
- [Lecture 11](assets/11.neon-datagrip-import-query.pdf.pdf) - neon-datagrip-import-query
- [Lecture 12](assets/12.neondb_setup.pdf.pdf) - neondb_setup
- [Lecture 13](assets/13.security.pdf.pdf) - security
- [Lecture 14](assets/14.constraints-all.pdf.pdf) - constraints-all
- [Lecture 15](assets/15.sql_time.pdf.pdf) - sql_time
- [Lecture 16](assets/16.pharmacies_er.pdf.pdf) - pharmacies_er
- [Lecture 17](assets/17.midtermCSC370_old_er_to_tables_sol.pdf.pdf) - midtermCSC370_old_er_to_tables_sol
- [Lecture 18](assets/18.midtermCSC370_old.pdf.pdf) - midtermCSC370_old
- [Lecture 19](assets/19.midterm_review.pdf.pdf) - midterm_review
- [Lecture 20](assets/20.midterm-review-handwritten.pdf.pdf) - midterm-review-handwritten
- [Lecture 21](assets/21.midtermCSC370_old_solQ3.pdf.pdf) - midtermCSC370_old_solQ3
- [Lecture 22](assets/22.sql_window.pdf.pdf) - sql_window
- [Lecture 23](assets/23.storage.pdf.pdf) - storage
- [Lecture 24](assets/24.btrees.pdf.pdf) - btrees
- [Lecture 25](assets/25.queryeval.pdf.pdf) - queryeval
- [Lecture 26](assets/26.concurrency.pdf.pdf) - concurrency
- [Lecture 27](assets/27.concurrency_lock_types.pdf.pdf) - concurrency_lock_types
- [Lecture 28](assets/28.recovery.pdf.pdf) - recovery
- [Lecture 29](assets/29.bcnf.pdf.pdf) - bcnf
